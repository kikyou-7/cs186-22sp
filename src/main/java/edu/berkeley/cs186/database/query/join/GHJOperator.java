...
public class GHJOperator extends JoinOperator {
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        ...
        for (Record record : records) {
            DataBox col = record.getValue(getLeftColumnIndex());
            if (!left) {
                col = record.getValue(getRightColumnIndex());
            }
            int hash = HashFunc.hashDataBox(col, pass);
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        Map<DataBox, ArrayList<Record>> hashTable = new HashMap<>();

        //build hashTable stage
        ...
        for (Record record : probeRecords) {
            ...
            if (hashTable.containsKey(key)) {
                ArrayList<Record> ret = hashTable.get(key);
                for (Record it : ret) {
                    joinedRecords.add(it.concat(record));
                }
            }
        }
    }
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        for (int i = 0; i < leftPartitions.length; i++) {
            ...
            rightPartitions[i].getNumPages() <= this.numBuffers - 2) {
                ...
                this.run(leftPartitions[i].getScanOperator(),
                        rightPartitions[i].getScanOperator(), pass+1);
            }
        }
    }
    ...
    public static Pair<List<Record>, List<Record>> getBreakSHJInputs() {
        ...
        //最多 (B-1)*(B-2)*8 = 5 * 4 * 8 = 160
        for (int i = 0; i < 161; i++) {
            leftRecords.add(createRecord(i));
            rightRecords.add(createRecord(i+1));
        }
